-- ============================================================================
-- Миграция #9: RPC для детальной карточки товара и каталога категорий
-- (исправлена: LIMIT 5 работает корректно через подзапрос)
-- ============================================================================
...
BEGIN
    ...
    -- Последние 5 поступлений (строк прихода) с инфой о накладной
    WITH latest_items AS (
        SELECT ri.id AS item_id, ri.quantity, ri.price, ri.total,
               r.id AS receipt_id, r.receipt_number, r.created_at AS receipt_date,
               r.supplier_id, r.warehouse_id
    ...
    SELECT COALESCE(JSONB_AGG(jsonb_build_object(
        ...
        'receipt_number', li.receipt_number,
        'receipt_date', li.receipt_date,
        'supplier_id', s.id,
        ...
        'price', li.price,
        'total', li.total
    ) ORDER BY li.receipt_date DESC), '[]'::jsonb) INTO v_receipts
    FROM latest_items li
    LEFT JOIN household_chemicals.suppliers s ON s.id = li.supplier_id
    LEFT JOIN household_chemicals.warehouses w ON w.id = li.warehouse_id;
