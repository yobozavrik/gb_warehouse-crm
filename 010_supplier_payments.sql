...
-- Create trigger_set_updated_at function if not exists
CREATE OR REPLACE FUNCTION household_chemicals.trigger_set_updated_at()
...
-- Ensure auth_user_id has a unique constraint for FK reference
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'users_auth_user_id_key'
    ) THEN
        ALTER TABLE household_chemicals.users ADD CONSTRAINT users_auth_user_id_key UNIQUE (auth_user_id);
    END IF;
END $$;
...
DROP FUNCTION IF EXISTS household_chemicals.rpc_suppliers_with_stats();
